...
@login_required
def currenttodos(request):  # Актуальные задачи
    todos = Todo.objects.filter(user=request.user, date_completed__isnull=True)
    return render(request, 'todo/currenttodos.html', {'todos': todos})


@login_required
def createtodo(request):  # создать задачу
    if request.method == "GET":
...
@login_required
def viewtodo(request, todo_pk):  # Страница с динамическими ключами
    todo = get_object_or_404(Todo, pk=todo_pk)
    if request.method == "GET":
        form = TodoForm(instance=todo)  # instance = это экземпляр класса
...
@login_required
def completetodo(request, todo_pk):  # добавляем задачу
...
@login_required
def completedtodos(request):  # выполненные задачи
