def add_title(title):
    def wrapper(func):
        def wrap(*args, **kwargs):
            print(f' {title} '.center(50, '='))
            output = func(*args, **kwargs)
            print('=' * 50)
            return output
        return wrap
    return wrapper


class UserInterface:
    @add_title('Ввод пользовательских данных')
    def wait_user_answer(self):
        # print(' Ввод пользовательских данных '.center(50, '='))  # убрали в декоратор
        print('Действие со статьями')
        print('1 - создание статьй'
              '\n2 - просмотр статей'
              '\n3 - просмотр определенной статьй'
              '\n4 - удаление статьй'
              '\nq - выход из программы')
        user_answer = input('Выберите вариант действия: ')
        # print('=' * 50)
        return user_answer

    @add_title('Создание статьй')
    def add_user_article(self):
        dict_article = {
            'название': None,
            'автор': None,
            'количество страниц': None,
            'описание': None,
        }
        for key in dict_article:
            dict_article[key] = input(f'Введите {key} статьи: ')
        return dict_article

    def show_all_articles(self, articles):
        print(' Список статей '.center(50, '='))
        for ind, article in enumerate(articles, start=1,):  # Метод записать
            print(f'{ind}, {article}')
        print('=' * 50)

    @add_title('Ввод названия статьй')
    def get_user_article(self):
        user_article = input('Введите название статьй: ')
        return user_article

    @add_title('Просмотр статьй')
    def show_single_article(self, article):
        for key in article:
            print(f'{key} статьй: - {article[key]}')

    @add_title('Сообщение об ошибке')
    def show_incorrect_title_error(self, user_title):
        print(f'Статья с названием {user_title} не существует')

    @add_title('Удаление статьй')
    def remove_single_article(self, article):
        print(f'Статья {article} была удалена')

    @add_title('Сообщение об ошибке')
    def show_incorrect_answer_error(self, answer):
        print(f'Варианта {answer} не существует')
